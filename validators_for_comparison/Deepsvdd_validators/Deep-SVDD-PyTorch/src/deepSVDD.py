...
class DeepSVDD(object):
    ...
    def single_test(self, inputs, net):
        """
        Perform a single test on an input to calculate the distance from the hypersphere center.
        
        Args:
            inputs: Input data (a single sample or a batch).
            net: The trained network \phi.

        Returns:
            The output embeddings of the network.
        """
        net.eval()  # Set the network to evaluation mode
        with torch.no_grad():
            outputs = net(inputs)  # Get the embedding
        return outputs
