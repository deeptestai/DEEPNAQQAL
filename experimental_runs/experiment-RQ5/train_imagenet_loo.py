...
dest_folder = f"results/results_ool2_img"
...
for leave_one_out_folder in folders:
   ...
   # acc = accuracy_score(y_true, y_pred)
    ...
    with open(f"{dest_folder}/SEED{RANDOM_SEED}_AUG{UPSAMPLE_TO}_{leave_one_out_folder}.txt", "w") as f:
         ...
         cm_llm = confusion_matrix(y_test.cpu().numpy(), y_test_llm.cpu().numpy())  # Add llm confusion matrix
         acc_llm = accuracy_score(y_test.cpu().numpy(), y_test_llm.cpu().numpy())
         cm_llm2 = confusion_matrix(y_test.cpu().numpy(), y_test_llm2.cpu().numpy())  # Add llm2 confusion matrix
         acc_llm2 = accuracy_score(y_test.cpu().numpy(), y_test_llm2.cpu().numpy())
         cm_llm2 = confusion_matrix(y_test.cpu().numpy(), y_test_llm2.cpu().numpy())  # Add llm3 confusion matrix
         acc_llm2 = accuracy_score(y_test.cpu().numpy(), y_test_llm2.cpu().numpy())
