...
# Remove .npy from SAMPLE column in df1
#df1["SAMPLE_cleaned"] = df1[sample_col].str.replace(".npy", "", regex=False).str.strip()
#andle NaN values in df1 before mapping
...
print("Unique ID values in df2:", df2["ID_cleaned"].unique()[:10])

# Add the new column 'ID/OOD Selforacle 99999' and populate it where SAMPLE matches df2 ID
df2["ID/OOD Selforacle 99"] = df2["ID_cleaned"].map(df1.set_index("SAMPLE_cleaned")["ID/OOD"])

# Check if mapping worked correctly
print("Number of mapped values:", df2["ID/OOD Selforacle 99"].notna().sum())
