...
dest_folder = f"results/{DATASET_NAME}/results_{QM_POLICY}/test_ool70"
...
df = pd.read_csv(f"experimental_data/TIGvalidity - {DATASET_NAME.upper()}_SURVEY.csv")
subfolders = ['dj', 'dx', 'dlf', 'ox', 'sv']
...
def process_folder(folder_name):
   ...
   # y_llm2 = torch.Tensor(test_data["y_llm2"]).to(torch.long)
    ...
    all_labels = np.concatenate([y, y_daiv, y_so, y_deepsvdd, y_llm, y_llm2])
    ...
    if num_ood == 1:
       print("⚠️ Only one OOD instance found. Keeping it in the training set.")
   ...
   # if len(unique_combinations) == 1:  
    #   print("⚠️ Skipping folder: Only one unique class with a single instance.")
     #  return  # Skip the folder
   # if any(counts < 2):  # If any class has only one instance
    #    print("⚠️ Warning: Some classes have only one instance. Disabling stratify for this folder.")
   ...
   # else:
    ...
    with open(f"{dest_folder}/SEED{RANDOM_SEED}_AUG{UPSAMPLE_TO}_{folder_name}.txt", "w") as f:
        ...
        cm_llm = confusion_matrix(y_test, y_test_llm,labels=[0,1])  # Add DeepSVDD confusion matrix
        acc_llm = accuracy_score(y_test, y_test_llm)
        cm_llm2 = confusion_matrix(y_test, y_test_llm2,labels=[0,1])  # Add DeepSVDD confusion matrix
        ...
        # Write confusion matrix and accuracy comparison
        f.write(f"{'':5}   {'Ours':^10}|{'DAIV':^10}|{'SO':^10}|{'DeepSVDD':^10}|{'LLM':^10}|{'LLM2':^10}")
        f.write("\n")
        f.write(f"{'TP':5} = {tp:^10}|{tp_daiv:^10}|{tp_so:^10}|{tp_deepsvdd:^10}|{tp_llm:^10}|{tp_llm2:^10}")
        f.write("\n")
        f.write(f"{'TN':5} = {tn:^10}|{tn_daiv:^10}|{tn_so:^10}|{tn_deepsvdd:^10}|{tn_llm:^10}|{tn_llm2:^10}")
        f.write("\n")
        f.write(f"{'FP':5} = {fp:^10}|{fp_daiv:^10}|{fp_so:^10}|{fp_deepsvdd:^10}|{fp_llm:^10}|{fp_llm2:^10}")
        f.write("\n")
        f.write(f"{'FN':5} = {fn:^10}|{fn_daiv:^10}|{fn_so:^10}|{fn_deepsvdd:^10}|{fn_llm:^10}|{fn_llm2:^10}")
        f.write("\n")
        f.write(f"{'Acc':5} = {acc:^10.3}|{acc_daiv:^10.3}|{acc_so:^10.3}|{acc_deepsvdd:^10.3}|{acc_llm:^10.3}|{acc_llm2:^10.3}")
        f.write("\n")
