...
dest_folder = f"results/results_ool_img"
...
### Data Preprocessing
# Data Preprocessing
data = create_dataset_dict("experimental_data/TIGvalidity - {DATASET_NAME.upper()}_SURVEY.csv",dataset_name="imagenet",question_marks=QM_POLICY)
...
folders = ['sv', 'ox', 'dlf', 'dx']
# Encode the tool names
le_y = preprocessing.LabelEncoder()
le_y.fit(np.concatenate([y, y_so, y_daiv, y_deepsvdd, y_llm]))  
...
for leave_one_out_folder in folders:
   ...
   # assert test_indices.shape[0] == X.shape[0], f"Mask shape {test_indices.shape[0]} doesn't match X {X.shape[0]}"
    ...
    for i, (train_idx, val_idx) in enumerate(skf.split(X.cpu().numpy(), y.cpu().numpy())):
        ...
        model_ft, data_transforms = vgg16_transfer(num_classes=2)
    ...
    if SAVE_MODEL:
        model_dest_folder = f"validator_models/{DATASET_NAME}/{QM_POLICY}/img_ool"
    ...
    best_model, _ = vgg16_transfer(num_classes=2)
