...
IMG_SIZE = 28
DATASET_NAME = "mnist"
dest_folder = "results/results_ool_mnist"
...
le_y.fit(np.concatenate([y, y_so, y_daiv, y_deepsvdd,y_llm,y_llm2]))  
   ...
   # assert test_indices.shape[0] == X.shape[0], f"Mask shape {test_indices.shape[0]} doesn't match X {X.shape[0]}"
    ...
    for i, (train_idx, val_idx) in enumerate(skf.split(X.cpu().numpy(), y.cpu().numpy())):
        ...
        if len(unique_classes) < 2:
           print(f"⚠️ Skipping Split {i + 1} due to single-class training data.")
