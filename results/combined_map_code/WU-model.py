...
# Group by Dataset and AUG to apply the Mann-Whitney U test
for (dataset, Model), group in data.groupby(['Dataset', 'Model']):
    ...
    if len(best_acc) > 1 and len(second_best_acc) > 1:
        try:
            ...
            print(f"Error for Dataset: {dataset}, AUG: {aug}: {e}")

# Convert results to a DataFrame
results_df = pd.DataFrame(results)

# Save results to a CSV file
results_file = "mwu_results_with_best_and_second_best_ool70.csv"
