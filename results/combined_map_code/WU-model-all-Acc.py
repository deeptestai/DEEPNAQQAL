...
data = pd.read_csv("./RQ3_results.csv")
...
for (dataset, Model), group in data.groupby(['Dataset', 'Model']):
    ...
    # Compute mean accuracy for sorting
    mean_accuracies = {model: acc.mean() for model, acc in accuracy_dict.items()}
    ...
    print(f"Best Model: {best_model}, Acc: {best_acc}")
    ...
    for i in range(1, len(sorted_models)):
        ...
        if len(best_acc) > 1 and len(compare_acc) > 1:
            try:
                ...
                print(f"Error for Dataset: {dataset}, Model: {Model}, Comparison: {best_model} vs {compare_model}: {e}")

# Convert results to a DataFrame
results_df = pd.DataFrame(results)

# Save results to a CSV file
results_file = "mwu_RQ3img_all.csv"
