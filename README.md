...
> These files and folders serve as the central location for both raw and processed data used in evaluations.
---
- **experimental_runs**
...
Each subfolder includes the **main SL validator training scripts** relevant to its research question. These scripts are used to train and evaluate validators for comparison on a research question-wise basis across datasets and validators.
...
- **validators_for_comparison**  
  ...
  - `llm_validators` — scripts for large language model (LLM)-based validators.  
    - Contains subfolders for **GPT-4** and **GPT-5** validators.  
    - GPT-5 scripts may also be used for generating additional validation outputs.  
  - `reconst_based_validators` — scripts for reconstruction-based validators  
---
- **generated_images**  
  Includes image datasets such as MNIST, SVHN, and ImageNet.  
...
> All compiled CSVs and scripts in this folder are generated after running SL validator experiments and are used for evaluation and comparison of validator performance.
